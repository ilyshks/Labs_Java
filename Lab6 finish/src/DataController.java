...
public class DataController {
    ...
    public void addVehicles(Scanner in){
        ...
        for(int i = 0; i < cntVehicles; i++){
            ...
            in.nextLine();
            NewAuto.createAndShowGui();
            while (!NewAuto.getIsDone()){
                try {
            ...
            String[] data = NewAuto.getData();
            String mark = data[0];
            String vehicleModel = data[1];
            double maxWeight = Double.parseDouble(data[2]);
...
//            double topSpeed = inputConsolePositiveDouble(in);
    ...
    public void delVehicle(Scanner in){
        ...
        int index = inputLimitedInt(in, 1, model.getVehicles().size());
        model.delVehicle(index - 1);
    }
